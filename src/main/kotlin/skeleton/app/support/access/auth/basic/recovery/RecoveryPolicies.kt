...
object RecoveryPolicies {
    fun assertValidToken(tokenOptional: Optional<RecoveryToken>) {
        if (tokenOptional.isEmpty) {
            throw ResponseStatusException(HttpStatus.BAD_REQUEST, "Password recovery session not found")
        }
        ...
        if (token.recoveryDate.before(Date.from(Instant.now().minus(duration)))) {
            throw ResponseStatusException(HttpStatus.BAD_REQUEST, "Password recovery session has been expired!")
        }
    }
}